...
function positionOf(moment: TimelineMoment, fallback: number): number {
  return typeof moment.position === "number" && Number.isInteger(moment.position)
    ? moment.position
    : fallback;
}

export function compareTimelineMoments(left: TimelineMoment, right: TimelineMoment): number {
  return (
    timeOf(left, 0) - timeOf(right, 0) ||
    positionOf(left, 0) - positionOf(right, 0) ||
    left.id.localeCompare(right.id)
  );
}

/** Canonical cross-host order: signed time, stable position, then id. */
export function canonicalTimelineOrder(timeline: TimelineMoment[]): TimelineMoment[] {
  return [...timeline].sort(compareTimelineMoments);
}
