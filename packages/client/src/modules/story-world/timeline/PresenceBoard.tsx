...
export function PresenceBoard({
  ...
  momentId: string;
  onPatch: (nodeId: string, placeId: string) => void;
}) {
  const { t } = useI18n();
  const [selectedChip, setSelectedChip] = useState<string | null>(null);
  const [dropTarget, setDropTarget] = useState<string | null>(null);

  const editorFor = (node: FigureNode) =>
    presenceFieldEditor(node.id, presence, timeline, momentId);
  const assign = (nodeId: string, laneId: string) =>
    onPatch(nodeId, laneId === UNCHANGED ? "" : laneId);

  const lanes = [
    { id: UNCHANGED, name: t("unchangedNoPlace") },
    ...
    <div className="presence-board">
      ...
      >
        {nodes.map((node) => {
          ...
          const inheritedName =
            !editor.placeId && editor.inheritedPlaceId
