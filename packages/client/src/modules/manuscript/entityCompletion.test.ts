...
describe("name completion, one edit per five characters", () => {
  ...
  it("treats non-ASCII spelling as edits without runtime-specific transliteration", () => {
    expect(suggest("Muller", ["Müller"])).toBe("Müller");
    expect(suggest("Mueller", ["Müller"])).toBeNull();
    expect(suggest("Strassen", ["Straßental"])).toBeNull();
    expect(suggest("Ubermann", ["Übermann"])).toBeNull();
  });
...
describe("name comparison", () => {
  it("folds ASCII case and preserves non-ASCII code points", () => {
    expect(foldName("Müller")).toBe("müller");
    expect(foldName("Straße")).toBe("straße");
    expect(foldName("Étienne")).toBe("Étienne");
  });
