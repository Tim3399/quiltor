...
describe("adaptive UI primitives", () => {
  ...
  it("keeps reduced-motion behavior in the shared motion foundation", () => {
    const motionCss = readFileSync(
      join(process.cwd(), "packages/client/src/design/motion.css"),
      "utf8",
    );
    expect(motionCss).toContain("@media (prefers-reduced-motion: reduce)");
    expect(motionCss).toContain("animation-duration: var(--motion-minimal) !important");
    expect(motionCss).toContain("animation-iteration-count: 1 !important");
    expect(motionCss).toContain("transition-duration: var(--motion-minimal) !important");
  });
});
