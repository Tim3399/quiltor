...
backend/llm/shared/contract.py sends: GET /health, POST /v1/chat/completions
...
backend/llm/installer.py (server.py offers this interactively on first
launch, or run `python3 -m backend.llm.installer --runtime mlx` directly).
It is spawned as a subprocess by
backend/llm/runtimes/mlx.py, the same way llama-server is spawned by
backend/llm/runtimes/llamacpp.py -- backend/assistant.py never imports
mlx/mlx_lm/llguidance directly, so a Windows/Linux install of Quiltor never
needs them.

Usage:
    python3 scripts/llm-runtime/mlx_bridge.py --model /path/to/mlx/model/dir --port 11435
"""
