...
const created = await page.request.post(`${baseUrl}/api/worlds/create`, { data: { title: 'Der gläserne Atlas', gitUrl: '' } });
const { world } = await created.json();
await page.request.post(`${baseUrl}/api/worlds/open`, { data: { id: world.id } });

const manuscript = {
  chapters: [
    { id: 'c-arrival', title: 'Die Ankunft', body: 'Der Morgen lag still über dem Hafen. Zwischen den Masten schimmerte das Archiv wie eine Erinnerung aus Glas.\n\nMara blieb am Ende des Stegs stehen. Heute würde sie erfahren, weshalb die Karten seit drei Nächten ihre Linien veränderten.', note: 'Die Unruhe der Stadt nur andeuten. Das Archiv als stillen Gegenpol etablieren.' },
    { id: 'c-archive', title: 'Das Archiv', body: 'Im Lesesaal wartete bereits der Hüter der Karten.', note: 'Erstes Zusammentreffen mit der Kartographengilde.' },
    { id: 'c-storm', title: 'Vor dem Sturm', body: '', note: 'Konflikt zwischen Gilde und Hafenrat zuspitzen.' },
    { id: 'c-crossing', title: 'Die Überfahrt', body: '', note: '' },
  ],
  words: [{ w: 'Lichtsaum', d: 'Schimmernde Grenze auf alten Karten' }, { w: 'Gezeitenarchiv', d: 'Zentrales Archiv der Stadt' }],
  zeichenAktiv: ['…', '—', '»', '«'],
};
const figures = {
  nodes: [
  ...
  edges: [
    { id: 'e1', from: 'mara', to: 'iven', label: 'misstraut', gerichtet: true, style: 'solid', versions: [{ momentId: 't2', label: 'arbeitet mit', active: true }, { momentId: 't3', label: 'vertraut', active: true }] },
    ...
    { id: 'e4', from: 'gilde', to: 'iven', label: 'beauftragt', gerichtet: true, style: 'dashed', versions: [{ momentId: 't3', label: 'verfolgt', active: true, style: 'blood' }] },
    { id: 'e5', from: 'mara', to: 'lumen', label: 'verbunden', gerichtet: false, style: 'solid' },
    { id: 'e6', from: 'atlas', to: 'gilde', label: 'beansprucht', gerichtet: true, style: 'dashed' },
  ],
  timeline: [
    { id: 't1', title: 'Ankunft im Nordhafen', date: '1847-09-03', note: 'Mara erreicht die Stadt und sucht das Archiv.' },
    { id: 't2', title: 'Öffnung des Atlas', date: '1847-09-06', note: 'Die erste verborgene Route erscheint.' },
    { id: 't3', title: 'Bruch mit der Gilde', date: '1847-09-11', note: 'Die Gilde erklärt Iven und Mara zu Verrätern.' },
    { id: 't4', title: 'Die Überfahrt', date: '1847-09-14', note: 'Aufbruch durch den Kartennebel.' },
  ],
};

await page.request.post(`${baseUrl}/api/manuscript`, { data: manuscript, headers: { 'If-Match': '"0"' } });
