...
tests/python/test_server_auth.py patches `auth.discover`: the endpoint runs as
...
token endpoints a login goes through (src/quiltor/infrastructure/backup/login.py), and the
...
class FakeIssuer:
    ...
    def __init__(self) -> None:
        ...
        self.kid = "test-rsa-key"
        self.token_kid = self.kid
    ...
    def jwk(self) -> dict:
        ...
        return {
            ...
            "use": "sig",
            "alg": "RS256",
            "n": self._uint(numbers.n),
            "e": self._uint(numbers.e),
        }

    def _grant(self, scope: str, *, nonce: str = "") -> dict:
        ...
        return {
            ...
            "id_token": jwt.encode(
                claims,
                self.signing_key_override or self._signing_key,
                algorithm="RS256",
                headers={"kid": self.token_kid},
            ),
        }
    ...
    def start(self) -> "FakeIssuer":
        ...
        class Handler(http.server.BaseHTTPRequestHandler):
            ...
            def do_GET(self) -> None:
                ...
                if parts.path == "/jwks":
                    return self._json({"keys": [issuer.jwk()]})
            ...
            def _token(self, fields: dict) -> None:
                ...
                if grant == "authorization_code":
                    ...
                    if (
                        ...
                        return self._json({"error": "invalid_grant"}, 400)
                    return self._json(issuer._grant(pending["scope"], nonce=pending["nonce"]))
