...
# than taking from the request -- see services/backup-server/server.py::_account.
...
class RestoreTest(unittest.TestCase):
    def setUp(self):
        ...
        self.authorization = BackupAuthorization(self.endpoint, TOKEN)
    ...
    def _machine(self, name: str):
        ...
        store = SnapshotStore(
            home / "history",
            HttpRemoteBackupGateway(self.endpoint, capabilities),
        )
        return store, home
    ...
    def _seed_and_upload(self):
        ...
        conn.commit()
        conn.close()
        result = store.commit(ctx, "Erster Stand", push=True, authorization=self.authorization)
    ...
    def test_endpoint_lists_worlds_with_their_titles(self):
        ...
        found = remote.worlds(self.endpoint, self.authorization)
    ...
    def test_a_world_never_backed_up_is_not_listed(self):
        ...
        self.assertEqual(remote.worlds(self.endpoint, self.authorization), [])
    ...
    def test_restoring_onto_a_machine_that_has_never_seen_the_world(self):
        ...
        entry = remote.snapshots(ctx_b, self.authorization)[-1]
        store_b.restore(
            ctx_b,
            entry,
            fetch=lambda digest: remote.fetch_blob(ctx_b, digest, self.authorization),
        )
