...
def fake_interaction_logger(question, response=None, error="", db_path=None):
...
class AssistantJobRunnerTests(unittest.TestCase):
    def test_runner_is_lazy_until_started_or_used(self):
        with tempfile.TemporaryDirectory() as tmp_name:
            data = Path(tmp_name)
            runner = AssistantJobRunner(FakeAssistant(), data)
            try:
    ...
    def test_missing_world_fails_without_recreating_the_world_database(self):
        with tempfile.TemporaryDirectory() as tmp_name:
            data = Path(tmp_name)
            runner = AssistantJobRunner(
                FakeAssistant(),
                data,
                interaction_logger=fake_interaction_logger,
            )
            try:
                job, _ = runner.submit(
                    owner_sub="user",
