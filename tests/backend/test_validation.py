...
class ManuscriptMarkValidationTests(unittest.TestCase):
    ...
    def manuscript(self, marks):
        return {"chapters": [{"id": "c1", "title": "", "body": "Mara und Bela", "note": "", "marks": marks}]}
    ...
    def test_accepts_ranges_inside_the_body_including_overlapping_kinds(self):
        ...
        self.assertTrue(valid_manuscript(self.manuscript([
            {"from": 0, "to": 13, "kind": "bold"}, {"from": 5, "to": 8, "kind": "italic"},
        ])))
        # Aneinandergrenzend ist erlaubt, wie bei den Erwähnungen.
        self.assertTrue(valid_manuscript(self.manuscript([
            {"from": 0, "to": 4, "kind": "bold"}, {"from": 4, "to": 8, "kind": "bold"},
        ])))

    def test_rejects_unknown_kinds_bad_bounds_and_overlap_within_one_kind(self):
        self.assertFalse(valid_manuscript(self.manuscript([{"from": 0, "to": 4, "kind": "underline"}])))
        ...
        self.assertFalse(valid_manuscript(self.manuscript([{"from": 0, "to": 4.5, "kind": "bold"}])))
        self.assertFalse(valid_manuscript(self.manuscript([
            {"from": 0, "to": 6, "kind": "bold"}, {"from": 3, "to": 9, "kind": "bold"},
        ])))
...
class MarkdownMirrorTests(unittest.TestCase):
    ...
    def test_writes_markers_for_the_ranges_and_leaves_plain_text_alone(self):
        ...
        self.assertEqual(markdown_body("Mara und Bela", [{"from": 0, "to": 4, "kind": "bold"}]), "**Mara** und Bela")
        self.assertEqual(markdown_body("Mara und Bela", [{"from": 9, "to": 13, "kind": "italic"}]), "Mara und *Bela*")
        self.assertEqual(markdown_body("Mara", [{"from": 0, "to": 4, "kind": "bold"}, {"from": 0, "to": 4, "kind": "italic"}]), "***Mara***")

    def test_keeps_the_markers_off_whitespace_and_out_of_paragraph_breaks(self):
        self.assertEqual(markdown_body("Mara und Bela", [{"from": 4, "to": 13, "kind": "italic"}]), "Mara *und Bela*")
        self.assertEqual(markdown_body("Eins\n\nZwei", [{"from": 0, "to": 10, "kind": "bold"}]), "**Eins**\n\n**Zwei**")
