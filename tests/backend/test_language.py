...
class LanguageServiceTests(unittest.TestCase):
    ...
    def test_status_install_and_normalized_lookups(self):
        ...
        self.assertIn("rasch", self.service.lookup("de-DE", "synonyms", "SCHNELL")["results"][0]["values"])
        self.assertIn("house", self.service.lookup("de-DE", "translation", "Haus")["results"][0]["values"])
        self.assertIn("Haus", self.service.lookup("en-GB", "translation", "house")["results"][0]["values"])
    ...
    def test_missing_data_and_invalid_requests_are_explicit(self):
        with self.assertRaises(FileNotFoundError): self.service.lookup("de-DE", "dictionary", "Haus")
        self.service.install()
        for language, mode, query in (("fr", "dictionary", "mot"), ("de-DE", "bad", "Haus"), ("de-DE", "dictionary", "")):
            with self.assertRaises(ValueError): self.service.lookup(language, mode, query)

    def test_outdated_database_requires_reinstallation(self):
        self.service.install()
        import sqlite3
        with sqlite3.connect(self.service.path) as conn: conn.execute("UPDATE metadata SET value='old' WHERE key='version'")
        self.assertFalse(self.service.status()["installed"])
    ...
    def test_provider_parsers_normalize_upstream_formats(self):
        ...
        wiki = list(parse_wiktionary([json.dumps({"lang_code": "de", "word": "gehen", "pos": "verb", "senses": [{"glosses": ["sich fortbewegen"]}]})]))
        self.assertEqual(wiki[0]["meaning"], "sich fortbewegen")
        tei = io.BytesIO(b'<TEI xmlns="http://www.tei-c.org/ns/1.0"><text><entry><form><orth>Haus</orth></form><gramGrp><pos>n</pos></gramGrp><sense><cit><quote>house</quote></cit></sense></entry></text></TEI>')
        self.assertEqual(list(parse_freedict(tei, "de-DE", "en-GB"))[0]["values"], ["house"])

    def test_checksum_validation_rejects_stale_data(self):
        path = Path(self.temp.name) / "source"; path.write_bytes(b"valid")
        self.assertTrue(validate_checksum(path, "sha256:ec654fac9599f62e79e2706abef23dfb7c07c08185aa86db4d8695f0b718d1b3"))
