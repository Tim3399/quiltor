...
class McpTest(unittest.TestCase):
    def test_server_advertises_only_read_and_proposal_tools(self):
        ...
        self.assertTrue({"list_elements", "list_relationships", "get_relationship_history", "list_timeline_moments", "get_board_layout", "validate_world"}.issubset(names))
    ...
    def test_structured_read_tools_return_complete_collections(self):
        ...
        with patch("mcp.quiltor_server._world", return_value=({}, figures)):
            ...
            self.assertEqual(call_tool("get_relationship_history", {"worldId": "world", "relationshipId": "e1"})["relationship"]["id"], "e1")
            self.assertEqual(call_tool("get_board_layout", {"worldId": "world"})["elements"][0]["x"], 10)
