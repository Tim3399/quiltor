...
def _proposal(arguments: dict[str, Any], figures: dict[str, Any], kind: str) -> dict[str, Any]:
    ...
    if kind == "update_element":
        ...
        return {
            ...
            "patch": {
                key: arguments[key]
                for key in ("name", "label", "sub", "profile")
                if key in arguments
            },
        }
    ...
    if kind == "create_relationship":
        if (
            arguments["from"] not in nodes
            or arguments["to"] not in nodes
            or arguments["from"] == arguments["to"]
        ):
    ...
    if kind == "create_timeline_moment":
        return {
            "kind": kind,
            "tempId": "new:moment:mcp",
            "moment": {
                key: arguments[key] for key in ("title", "date", "note") if key in arguments
            },
        }
    ...
    if kind == "set_presence":
        ...
        if (
            arguments["elementId"] not in nodes
            or arguments["placeId"] not in places
            or (arguments.get("momentId") and arguments["momentId"] not in moments)
        ):
        ...
        return {
            ...
            **({"momentId": arguments["momentId"]} if arguments.get("momentId") else {}),
        }
...
def call_tool(name: str, arguments: dict[str, Any]) -> Any:
    ...
    if name == "search_world":
        chunks = retrieve(
            build_knowledge(manuscript, figures),
    ...
    if name == "get_world_structure":
        return {
            ...
            "edges": figures.get("edges", []),
            "timeline": figures.get("timeline", []),
            "presence": figures.get("presence", []),
        }
    ...
    return {
        "proposal": _proposal(arguments, figures, mapping[name]),
    ...
    if method == "initialize":
        return {
            ...
            "result": {
                ...
                "serverInfo": {"name": "quiltor", "version": "2.0.0"},
