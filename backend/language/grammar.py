...
class LanguageToolManager:
    def __init__(self, data_dir: Path):
        self.root = data_dir / "language" / f"LanguageTool-{LANGUAGETOOL_VERSION}"
    ...
    def status(self) -> dict:
        ...
        return {
            ...
            "javaRequired": JAVA_MINIMUM, "externalConfigured": bool(configured_url),
            "externalEnabled": bool(configured_url and external_opt_in),
            "download": {"url": LANGUAGETOOL_URL, "checksum": f"sha256:{LANGUAGETOOL_SHA256}", "license": "LGPL-2.1-or-later"},
        }
    ...
    def install(self) -> dict:
        ...
        request = urllib.request.Request(LANGUAGETOOL_URL, headers={"User-Agent": "Quiltor local installer"})
        with urllib.request.urlopen(request, timeout=120) as response:
            payload = response.read()
        if hashlib.sha256(payload).hexdigest() != LANGUAGETOOL_SHA256:
            raise ValueError("LanguageTool checksum mismatch")
        parent = self.root.parent; parent.mkdir(parents=True, exist_ok=True)
        temporary = parent / f".{self.root.name}.installing"
        if temporary.exists(): shutil.rmtree(temporary)
        temporary.mkdir()
        try:
            with zipfile.ZipFile(io.BytesIO(payload)) as archive: _safe_extract(archive, temporary)
            extracted = temporary / self.root.name
            if not (extracted / "languagetool-server.jar").exists(): raise ValueError("LanguageTool server is missing")
    ...
    def _local_url(self) -> str:
        with self.lock:
            ...
            status = self.status()
            if not status["available"]: raise FileNotFoundError("LanguageTool ist nicht installiert oder Java fehlt")
            with socket.socket() as candidate:
                candidate.bind(("127.0.0.1", 0)); self.port = candidate.getsockname()[1]
            ...
            for _ in range(40):
                ...
                try:
                    with urllib.request.urlopen(f"http://127.0.0.1:{self.port}/v2/languages", timeout=.25): return url
                except OSError: time.sleep(.1)
            self.close(); raise RuntimeError("LanguageTool konnte nicht gestartet werden")

    def check(self, language: str, text: str, custom_words: list[str]) -> dict:
        if language != "de-DE" or not text or len(text) > 200_000: raise ValueError("invalid grammar check")
        ...
        request = urllib.request.Request(url, data=body, headers={"Content-Type": "application/x-www-form-urlencoded", "User-Agent": "Quiltor local grammar"})
        ...
        for index, match in enumerate(result.get("matches", [])):
            ...
            if start < 0 or length <= 0 or start + length > len(text) or text[start:start + length].casefold() in own: continue
            rule = match.get("rule") or {}; category = rule.get("category") or {}
            seed = f"{start}:{length}:{rule.get('id', '')}:{index}".encode()
            issues.append({"id": hashlib.sha1(seed).hexdigest()[:16], "from": start, "to": start + length,
                           "ruleId": str(rule.get("id", "")), "category": str(category.get("name", "")),
                           "message": str(match.get("message", ""))[:1000],
                           "replacements": [str(item.get("value", "")) for item in match.get("replacements", [])[:8] if item.get("value")]})
