...
def _sweep_progress(now: float) -> None:
    stale = [
        key for key, entry in _progress.items() if now - entry["updatedAt"] > _PROGRESS_TTL_SECONDS
    ]
    for key in stale:
        del _progress[key]
...
def update_progress(
    progress_id: str, done: int, label_key: str, label_params: dict[str, Any]
) -> None:
    now = time.time()
    with _progress_lock:
        _sweep_progress(now)
...
def _set_deterministic_message(
    parsed: dict[str, Any],
    key: str,
...
class AssistantRuntime:
    ...
    def complete(
        ...
        if forced:
            ...
            forced = _fit_to_budget(
                forced, self.url, MODEL_CONTEXT_TOKENS - CONTEXT_SAFETY_MARGIN, trace
            )
            trace.append(
                {
                    "step": "force_context",
        ...
        plan = (
            ...
            else self._plan(question, context)
        )
        ...
        context = pack_chunks(
            context_candidates, self.url, RUNTIME_CONFIG.forced_context_tokens, count_tokens, trace
        )
        ...
        def prompt_for(packed_json: str) -> tuple[str, int, int]:
            ...
            tokens = count_tokens(
                self.url, prompt + "".join(message["content"] for message in conversation) + content
            )
        ...
        if required - {item.get("kind") for item in parsed["proposals"]}:
            ...
            if deterministic_proposals:
                ...
                retry_prompt_tokens = prompt_tokens + count_tokens(
                    self.url, json.dumps(parsed, ensure_ascii=False) + repair_note
                )
        ...
        if MUTATION_REQUEST.search(question) and not parsed["proposals"]:
            ...
            if os.environ.get("QUILTOR_AI_DEBUG"):
                print(
                    f"  · AI forced proposal: {json.dumps(forced, ensure_ascii=False)}", flush=True
                )
        ...
        parsed["citations"] = list(
            dict.fromkeys(source for source in raw_citations if source in known)
        )
        ...
        if contract["audit"]:
            ...
            parsed["proposals"] = []
            parsed["citations"] = [chunk.id for chunk in context if chunk.kind == "relationship"][
                :12
            ]
            ...
            _set_deterministic_message(
                parsed, "taskIncompleteMissing", items=_missing_items(verification["missing"])
            )
    ...
    def _forced_proposal(
        ...
        if "create_element" in required:
            ...
            candidates = re.findall(
                r"\b[A-ZÄÖÜ][\wÄÖÜäöüß-]*(?:\s+[A-ZÄÖÜ][\wÄÖÜäöüß-]*)*", question
            )
            generic = {
                "lege",
                "erstelle",
        ...
        if required == {"update_element"} and node:
            ...
            patch = {
                "profile": {"notizen": (note.group(1).strip().rstrip(".") if note else question)}
            }
            return {"kind": "update_element", "elementId": node["id"], "patch": patch}
        if required == {"set_relationship_at_moment"}:
            edge = next(
                (item for item in edges if str(item.get("id", "")).casefold() in folded), None
            )
            if edge and moment:
        ...
        if "beziehung" in folded or "relationship" in folded:
            ...
            if len(matches) >= 2:
                label = (
                    "Besitzt" if re.search(r"\b(besitzt|gehört|owns?)\b", folded) else "Beziehung"
                )
                return {
...
def _fit_to_budget(
    chunks: list[Any], url: str, budget: int, trace: list[dict[str, Any]]
) -> list[Any]:
