...
AMBIGUOUS_REFERENCE = re.compile(r"\b(er|sie|es|ihn|ihm|ihr|seine|seiner|deren|dies(?:e|er|es)|dort|he|she|it|his|her|there)\b", re.I)


def resolve_reference(question: str, history: list[dict[str, Any]] | None, figures: dict[str, Any]) -> dict[str, Any] | None:
    ...
    explicit = [item for item in objects if str(item.get("name") or item.get("title") or "").casefold() in question.casefold()]
    ...
    for message in reversed(history or []):
        for ref in reversed(message.get("references") or []):
            plain = str(ref).split(":", 1)[-1] if str(ref).startswith(("element:", "timeline:")) else str(ref)
    ...
    if not candidates:
        ...
        candidates = explicit or [item for item in objects if str(item.get("name") or item.get("title") or "").casefold() in folded]
    if len(candidates) == 1:
