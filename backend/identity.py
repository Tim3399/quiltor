...
This module lives in `backend/`, not `backend/core/`: `core` is the HTTP-free
domain (enforced by tests/backend/test_core.py) and an identity reads cookies,
headers and query parameters. It also never imports `server` -- backend/ must
stay usable from the CLI and the MCP server, which start no HTTP server at all
...
from backend.pdf import RENDER_TOKEN_TTL, redeem_render_token

#: The session cookie's name. It lives here rather than in server.py because the
#: identity is what writes and reads it; server.py imports it from here.
...
class Identity:
    ...
    def resolve(self, handler) -> auth.SessionData | None:
        ...
        log in for itself.
        """
        session = auth.get_session(handler.get_cookie(SESSION_COOKIE))
        ...
        queue_session_cookie(handler, session_id, max_age=RENDER_TOKEN_TTL)
...
class LocalIdentity(Identity):
    ...
    MASTER_SUB = storage.LOCAL_OWNER

    login_url = None
    multi_user = False

    def __init__(self) -> None:
        """Take the token from the environment, or invent one for this process.
        ...
        """
        self.token = os.environ.get("QUILTOR_MASTER_TOKEN") or secrets.token_urlsafe(32)
        self._session_id = ""
    ...
    def resolve(self, handler) -> auth.SessionData | None:
        ...
        if header.lower().startswith("bearer "):
            if secrets.compare_digest(header[7:].strip(), self.token):
    ...
    def _master_session(self, handler) -> auth.SessionData | None:
        ...
        otherwise mint a fresh entry on every call, and auth.SESSIONS only drops
        ...
        with self._session_lock:
            ...
            if session is None:
                self._session_id = auth.create_session(self.MASTER_SUB, "", "")
                session = auth.get_session(self._session_id)
        queue_session_cookie(handler, self._session_id, max_age=auth.SESSION_TTL)
        return session
