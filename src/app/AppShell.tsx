...
function useNarrowBar() {
  ...
  useEffect(() => {
    ...
    const media = matchMedia(query), change = () => setNarrow(media.matches);
...
export function AppShell({ title, workspace, onWorkspace, navigationAvailable = false, navigationOpen = false, onNavigation, phase, error, retry, theme, onTheme, onSearch, onHistory, onSnapshot, onBackups, onAssistant, whoami, onLogout, version, children }: {
  ...
  return <div className="app-frame" data-workspace={workspace}>
    <header className="app-bar">
      <div className="app-bar__leading">{navigationAvailable && <button aria-pressed={navigationOpen} onClick={onNavigation} aria-label={t('toggleNavigation')} title={t('navigation')}><PanelLeft /></button>}<div className="brand" title={`${title} · ${PRODUCT_NAME}${version ? ` v${version}` : ''}`}><span>{title}</span><small>{PRODUCT_NAME}{version && ` · v${version}`}</small></div></div>
      ...
      <Popover anchorRef={overflowButton} open={overflowOpen} onClose={closeOverflow} label={t('menuActions')}><Menu label={t('menuActions')} onClose={closeOverflow}>
        {!saveStatusInBar && <div className="menu-save-status"><SaveStatus phase={phase} error={error} onRetry={retry} /></div>}
