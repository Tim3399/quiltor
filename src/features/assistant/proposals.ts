...
export function applyAssistantProposals(
  state: FigureState,
    ...
    if (proposal.kind === "create_element") {
      ...
      next.nodes = next.nodes.map((node) =>
        node.id === id
          ? {
              ...node,
              ...(typeof proposal.patch.name === "string"
                ? { name: proposal.patch.name.slice(0, 160) }
                : {}),
              ...(typeof proposal.patch.label === "string"
                ? { label: proposal.patch.label.slice(0, 160) }
                : {}),
              ...(typeof proposal.patch.sub === "string"
                ? { sub: proposal.patch.sub.slice(0, 1000) }
                : {}),
      ...
      next.edges = next.edges.map((edge) => {
        ...
        return {
          ...edge,
          versions: [
            ...(edge.versions || []).filter((item) => item.momentId !== momentId),
            version,
          ],
        };
      });
      ...
      next.presence = [
        ...next.presence.filter(
          (entry) => !(entry.elementId === elementId && (entry.momentId || undefined) === momentId),
        ),
        { id: uid("p"), elementId, placeId, ...(momentId ? { momentId } : {}) },
      ];
...
export function scopeAssistantProposals(
  proposals: AssistantProposal[],
  scope: string,
): AssistantProposal[] {
  const scoped = (value: string) =>
    value.startsWith("new:")
      ? `new:${value.startsWith("new:moment:") ? "moment:" : ""}${scope}:${value.replace(/^new:(moment:)?/, "")}`
      : value;
  return proposals.map((proposal) => {
    ...
    if (proposal.kind === "create_relationship")
      return {
        ...
        relationship: {
          ...
          from: scoped(proposal.relationship.from),
          to: scoped(proposal.relationship.to),
        },
      };
    if (proposal.kind === "set_relationship_at_moment")
      return {
        ...proposal,
        relationshipId: scoped(proposal.relationshipId),
        momentId: scoped(proposal.momentId),
      };
    if (proposal.kind === "mark_deceased")
      return {
        ...proposal,
        elementId: scoped(proposal.elementId),
        momentId: scoped(proposal.momentId),
      };
    if (proposal.kind === "set_presence")
      return {
        ...proposal,
        elementId: scoped(proposal.elementId),
        placeId: scoped(proposal.placeId),
        ...(proposal.momentId ? { momentId: scoped(proposal.momentId) } : {}),
      };
    return proposal;
  });
}
...
function arrangeNodes(nodes: FigureNode[], edges: FigureEdge[], strategy: "thematic" | "grid") {
  ...
  for (const group of groups) {
    const columns = Math.min(3, group.length),
      width = Math.max(1, columns) * GRID_X,
      rows = Math.ceil(group.length / columns),
      height = rows * GRID_Y;
    if (originX > 96 && originX + width > 1248) {
      originX = 96;
...
export function proposalLabel(
  ...
  const nodeName = (id: string) =>
    state.nodes.find((node) => node.id === id)?.name || id.replace("new:", `${t("newPrefix")}: `);
  if (proposal.kind === "create_element")
    return t("createElementLabel").replace("{name}", proposal.element.name || t("withoutName"));
  if (proposal.kind === "update_element")
    return t("updateElementLabel").replace("{name}", nodeName(proposal.elementId));
  if (proposal.kind === "create_timeline_moment")
    return t("createMomentLabel").replace("{title}", proposal.moment.title || t("untitled"));
  if (proposal.kind === "create_relationship")
    return t("createRelationshipLabel")
      .replace("{from}", nodeName(proposal.relationship.from))
      .replace("{to}", nodeName(proposal.relationship.to));
  if (proposal.kind === "set_relationship_at_moment")
    return t("updateRelationshipStateLabel").replace(
      "{label}",
      proposal.patch.label || t("statusFallback"),
    );
  if (proposal.kind === "arrange_elements") return t("rearrangeElementsLabel");
  if (proposal.kind === "set_presence")
    return `${nodeName(proposal.elementId)} → ${nodeName(proposal.placeId)}`;
