...
const RECHECK_MS = 1500, RECHECK_TRIES = 8;
...
export function SnapshotDialog({ onClose, flush }: { onClose: () => void; flush: () => Promise<void> }) {
  ...
  const [login, setLogin] = useState<BackupLoginStatus | null>(null), [loginError, setLoginError] = useState(''), [waiting, setWaiting] = useState(false), [rechecks, setRechecks] = useState(0);
  ...
  useEffect(() => {
    if (!checking) return;
    const again = setTimeout(() => { setRechecks(count => count + 1); void loadLogin(); }, RECHECK_MS);
  ...
  return <Dialog title={t('snapshotSave')} onClose={onClose}>
    {!status ? <p>{t('loadingBackupStatus')}</p> : !status.ok ? <div className="error-box" role="alert">{status.grund}</div> : <>
      ...
      <div className="dialog-actions"><button disabled={busy || !message.trim()} onClick={() => void send(false)}><Save />{t('saveOnly')}</button>
        {canSignIn || checking
          ? <button className="primary" disabled={busy || checking} onClick={() => void signIn()}><LogIn />{t('backupSignIn')}</button>
