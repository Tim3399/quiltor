...
export function GitDialog({ onClose, flush }: { onClose: () => void; flush: () => Promise<void> }) {
  ...
  return <Dialog title="Git · Arbeitsstand sichern" onClose={onClose}>
    {!status ? <p>Lade Git-Status …</p> : !status.ok ? <div className="error-box">{status.grund}</div> : <>
      <dl className="git-facts"><div><dt>Branch</dt><dd>{status.branch}</dd></div><div><dt>Ziel</dt><dd>{status.upstream || 'Nicht eingerichtet'}</dd></div><div><dt>Geändert</dt><dd>{status.anzahl || 0} Dateien</dd></div><div><dt>Nicht gepusht</dt><dd>{status.unveroeffentlicht || 0} Commits</dd></div></dl>
      ...
      <div className="dialog-actions"><button disabled={busy || !message.trim()} onClick={() => void send(false)}><GitCommit />Nur committen</button><button className="primary" disabled={busy || !message.trim() || !status.upstream} onClick={() => void send(true)}><UploadCloud />Committen & pushen</button></div>
