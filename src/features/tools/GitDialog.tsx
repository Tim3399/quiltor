...
export function GitDialog({ onClose, flush }: { onClose: () => void; flush: () => Promise<void> }) {
  ...
  return <Dialog title={t('gitSave')} onClose={onClose}>
    {!status ? <p>{t('loadingGitStatus')}</p> : !status.ok ? <div className="error-box">{status.grund}</div> : <>
      <dl className="git-facts"><div><dt>{t('branch')}</dt><dd>{status.branch}</dd></div><div><dt>{t('target')}</dt><dd>{status.upstream || status.remote || t('notConfigured')}</dd></div><div><dt>{t('changed')}</dt><dd>{status.anzahl || 0} {t('files')}</dd></div><div><dt>{t('notPushed')}</dt><dd>{status.unveroeffentlicht || 0} {t('commits')}</dd></div></dl>
