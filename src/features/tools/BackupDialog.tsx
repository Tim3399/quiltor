...
export function BackupDialog({ onClose, flush }: { onClose: () => void; flush: () => Promise<void> }) {
  ...
  useEffect(() => { void flush().then(api.backups).then(result => setItems(result.backups)).catch(reason => setError(String(reason))); }, [flush]);
  const restore = async () => { if (!selected) return; try { await api.restore(selected); location.reload(); } catch (reason) { setError(String(reason)); setSelected(null); } };
