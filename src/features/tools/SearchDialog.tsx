...
export function SearchDialog({ manuscript, figures, onClose, onWorkspace, onSelect, onCommand }: { manuscript: Manuscript; figures: FigureState; onClose: () => void; onWorkspace: (value: Workspace) => void; onSelect: (target: { workspace: Workspace; id: string }) => void; onCommand: (command: string) => void }) {
  ...
  const items = useMemo<CommandPaletteItem[]>(() => {
    ...
    const moments: CommandPaletteItem[] = (figures.timeline || []).map(moment => ({ id: `moment-${moment.id}`, label: moment.title, detail: moment.note || moment.date || t('moment'), keywords: [JSON.stringify(moment)], icon: <Clock3 />, requiresQuery: true, onSelect: () => { onWorkspace('timeline'); onSelect({ workspace: 'timeline', id: moment.id }); } }));
