...
const figure = (name: string, id = name.toLowerCase()): FigureNode => ({ id, x: 0, y: 0, type: 'person', name });

function suggest(typed: string, names: string[], vocabulary: string[] = []) {
  return entityCompletion(`Sie sah ${typed}`, `Sie sah ${typed}`.length, names.map(name => figure(name)), vocabulary)?.word ?? null;
}
