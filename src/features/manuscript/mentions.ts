...
const boundary = (text: string, from: number, to: number) => (!from || !wordCharacter.test(text[from - 1])) && (to === text.length || !wordCharacter.test(text[to]));

export function scanEntityMentions(text: string, nodes: FigureNode[], idFactory: () => string = () => crypto.randomUUID()) {
  ...
  const names = [...groups.keys()].sort((a, b) => b.length - a.length || a.localeCompare(b, 'de-DE'));
  const folded = text.toLocaleLowerCase('de-DE'), occupied: Array<[number, number]> = [];
  const mentions: EntityMention[] = [], ambiguous: AmbiguousMentionCandidate[] = [];
  for (const name of names) {
    ...
    while ((from = folded.indexOf(name, from)) >= 0) {
      ...
      if (!overlaps && boundary(text, from, to)) {
        ...
        if (matches.length === 1) mentions.push({ id: idFactory(), elementId: matches[0].id, from, to, surface: text.slice(from, to), source: 'deterministic', confidence: 1 });
        else ambiguous.push({ from, to, surface: text.slice(from, to), elementIds: matches.map(node => node.id) });
      }
...
export function mapMentions(mentions: EntityMention[], changes: ChangeSet, text: string) {
  return mentions.flatMap(mention => {
    ...
    changes.iterChanges((from, to) => { if ((from < mention.to && to > mention.from) || (from === to && from > mention.from && from < mention.to)) editedInside = true; });
    if (editedInside) return [];
    const from = changes.mapPos(mention.from, 1), to = changes.mapPos(mention.to, -1);
...
export function reconcileMentions(manuscript: Manuscript, nodes: FigureNode[]) {
  ...
  const chapters: Chapter[] = manuscript.chapters.map(chapter => {
    const mentions = (chapter.mentions || []).filter(mention => {
      const valid = known.has(mention.elementId) && mention.from >= 0 && mention.to <= chapter.body.length && mention.from < mention.to && chapter.body.slice(mention.from, mention.to) === mention.surface;
...
export function addDeterministicMentions(text: string, existing: EntityMention[], nodes: FigureNode[]) {
  const scanned = scanEntityMentions(text, nodes).mentions;
  const additions = scanned.filter(candidate => !existing.some(mention => candidate.from < mention.to && candidate.to > mention.from));
  return [...existing, ...additions].sort((a, b) => a.from - b.from || b.to - a.to);
}

export function replaceEntityMentions(manuscript: Manuscript, elementId: string, replacement: string): Manuscript {
