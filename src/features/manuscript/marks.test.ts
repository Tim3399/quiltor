...
import { bodyParagraphs, hasMark, mapMarks, markdownBody, markedSegments, marksAfterReplacement, normalizeMarks, toggleMark } from './marks';
...
describe('Auszeichnungen als Bereiche', () => {
  ...
  it('nimmt Auszeichnungen bei Textänderungen mit', () => {
    ...
    expect(mapMarks([bold(0, 5)], ChangeSet.of({ from: 8, insert: '!' }, 10), 11)).toEqual([bold(0, 5)]);
  ...
  it('schneidet den Text in Abschnitte gleicher Auszeichnung', () => {
    ...
    expect(markedSegments('Welt', 6, [bold(6, 10)])).toEqual([{ text: 'Welt', bold: true, italic: false }]);
    expect(markedSegments('Hallo', 0, [bold(0, 5), italic(0, 5)])).toEqual([{ text: 'Hallo', bold: true, italic: true }]);
  });
