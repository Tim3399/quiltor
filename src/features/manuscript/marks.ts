...
import type { TextMark, TextMarkKind } from '../../types';
...
export function normalizeMarks(marks: TextMark[] = [], length = Infinity): TextMark[] {
  ...
  for (const kind of MARK_KINDS) {
    const ranges = marks
      ...
      .map(mark => ({ kind, from: Math.max(0, Math.min(mark.from, length)), to: Math.max(0, Math.min(mark.to, length)) }))
    ...
    for (const range of ranges) {
      ...
      if (previous && previous.kind === kind && range.from <= previous.to) previous.to = Math.max(previous.to, range.to);
...
export function hasMark(marks: TextMark[] = [], from: number, to: number, kind: TextMarkKind): boolean {
 ...
 */
export function toggleMark(marks: TextMark[] = [], from: number, to: number, kind: TextMarkKind): TextMark[] {
...
export function mapMarks(marks: TextMark[] = [], changes: ChangeSet, length: number): TextMark[] {
  return normalizeMarks(marks.map(mark => ({ ...mark, from: changes.mapPos(mark.from, 1), to: changes.mapPos(mark.to, -1) })), length);
}
 ...
 */
export function marksAfterReplacement(marks: TextMark[] = [], from: number, to: number, insertedLength: number, bodyLength: number): TextMark[] {
  const delta = insertedLength - (to - from);
  const start = (position: number) => position <= from ? position : position >= to ? position + delta : from;
  const end = (position: number) => position <= from ? position : position >= to ? position + delta : from + insertedLength;
  return normalizeMarks(marks.map(mark => ({ ...mark, from: start(mark.from), to: end(mark.to) })), bodyLength);
}
 ...
 */
export function markedSegments(text: string, from: number, marks: TextMark[] = []): MarkedSegment[] {
  const relevant = normalizeMarks(marks).filter(mark => mark.to > from && mark.from < from + text.length);
  ...
  for (let index = 0; index < cuts.length - 1; index++) {
    ...
    const carries = (kind: TextMarkKind) => relevant.some(mark => mark.kind === kind && mark.from <= from + start && mark.to >= from + end);
    const segment = { text: text.slice(start, end), bold: carries('bold'), italic: carries('italic') };
    const previous = segments.at(-1);
    if (previous && previous.bold === segment.bold && previous.italic === segment.italic) previous.text += segment.text;
...
function emphasize(text: string, bold: boolean, italic: boolean): string {
  ...
  const open = `${bold ? '**' : ''}${italic ? '*' : ''}`, close = `${italic ? '*' : ''}${bold ? '**' : ''}`;
  return text.split(/(\n\s*\n)/).map(piece => {
