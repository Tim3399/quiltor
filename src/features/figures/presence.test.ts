...
import { figureJourney, journeyLegs, patchPresence, placeChronicle, placeJourney, presenceByPlace, presenceFieldEditor, prunePresence, resolvePresence, stopDateDiff } from './presence';
...
describe('presence resolution', () => {
  ...
  it('ignores an entry pointing at a moment that no longer exists', () => {
    const presence: PresenceEntry[] = [{ id: 'p1', elementId: 'ada', placeId: 'hafen', momentId: 'deleted-moment' }];
  ...
  it('field editor shows the own value, or the inherited place when left blank', () => {
    ...
    expect(presenceFieldEditor('ada', presence, timeline, 'before')).toEqual({ placeId: '', inheritedPlaceId: 'arcene' });
    const withOverride = patchPresence(presence, 'ada', 'betrayal', 'hafen');
    expect(presenceFieldEditor('ada', withOverride, timeline, 'betrayal')).toEqual({ placeId: 'hafen' });
    expect(presenceFieldEditor('ada', withOverride, timeline, 'after')).toEqual({ placeId: '', inheritedPlaceId: 'hafen' });
  });
  ...
  it('prunes entries whose element, place, or moment no longer exists', () => {
    ...
    expect(prunePresence(presence, [ada, arcene], timeline)).toEqual([{ id: 'p1', elementId: 'ada', placeId: 'arcene' }]);
  });
});
...
describe('place history', () => {
  ...
  it('builds stays per character at a place, including a synthetic death boundary', () => {
    ...
    expect(adaStays.map(stay => [stay.arrivedAt.index, stay.leftAt?.index, stay.died])).toEqual([[-1, 1, false], [2, undefined, false]]);
    const kaiStay = stays.find(stay => stay.elementId === 'kai');
    expect(kaiStay).toMatchObject({ arrivedAt: { index: 0 }, leftAt: { index: 2, momentId: 'after' }, died: true });
  });

  it('drops a zero-length stay when a character dies the same moment they arrive', () => {
    const milo: FigureNode = { id: 'milo', x: 0, y: 0, name: 'Milo', type: 'person', diedMomentId: 'betrayal' };
    const stays = placeJourney('arcene', [milo, arcene], [{ id: 'p6', elementId: 'milo', placeId: 'arcene', momentId: 'betrayal' }], timeline);
    expect(stays).toHaveLength(0);
  });

  it('finds no stays for a character who never visits the place', () => {
    expect(placeJourney('hafen', [ada, arcene, hafen], [{ id: 'p1', elementId: 'ada', placeId: 'arcene' }], timeline)).toHaveLength(0);
  });
  ...
  it('builds a chronicle whose occupants match presenceByPlace at every real moment, and drops the deceased', () => {
    ...
    expect(rowByIndex.get(-1)?.occupants.map(node => node.id).sort()).toEqual(['ada']);
    for (const moment of timeline) {
      const index = timeline.indexOf(moment);
      const expected = presenceByPlace([ada, dyingKai, arcene, hafen], presence, timeline, moment.id).get('arcene')?.map(node => node.id).sort() ?? [];
      expect(rowByIndex.get(index)?.occupants.map(node => node.id).sort() ?? []).toEqual(expected);
    }
...
describe('duration', () => {
  ...
  it('computes the day count between two dated stops', () => {
    const diff = stopDateDiff({ placeId: 'arcene', momentId: 'betrayal', index: 1 }, { placeId: 'hafen', momentId: 'after', index: 2 }, datedTimeline);
    expect(diff).toEqual({ days: 3, label: '3 Tage' });
  });

  it('reports an unknown duration when either stop is undated or is the base stop', () => {
    expect(stopDateDiff({ placeId: 'arcene', momentId: 'before', index: 0 }, { placeId: 'hafen', momentId: 'after', index: 2 }, datedTimeline).label).toBe('Dauer unbekannt');
    expect(stopDateDiff({ placeId: 'arcene', index: -1 }, { placeId: 'hafen', momentId: 'betrayal', index: 1 }, datedTimeline).label).toBe('Dauer unbekannt');
  });

  it('flags a stop pair whose dates run backwards', () => {
    const diff = stopDateDiff({ placeId: 'arcene', momentId: 'after', index: 2 }, { placeId: 'hafen', momentId: 'betrayal', index: 1 }, datedTimeline);
