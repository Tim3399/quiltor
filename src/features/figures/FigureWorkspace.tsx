...
function FigureWorkspaceInner({ state, onChange, targetId, onUndo, onRedo, canUndo = false, canRedo = false }: FigureWorkspaceProps) {
  ...
  return <section className="figure-workspace" aria-label="Figuren und Beziehungen">
    ...
    {selected && confirmDelete && <ConfirmDialog title="Element löschen" description={`„${selected.name}“ und alle zugehörigen Verbindungen werden entfernt. Rückgängig machen geht mit ⌘Z.`} confirmLabel="Element löschen" onConfirm={remove} onClose={() => setConfirmDelete(false)} />}
