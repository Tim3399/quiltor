...
function FigureWorkspaceInner({ state, onChange, targetId, onUndo, onRedo, canUndo = false, canRedo = false }: FigureWorkspaceProps) {
  ...
  return <section className="figure-workspace" aria-label={t('figuresAndRelationsLabel')}>
    <div className="context-bar">
      <div className="context-title"><strong>{t('figuresWorld')}</strong><span>{t('nElements', { n: state.nodes.length })} · {t('nRelationships', { n: state.edges.length })}</span></div>
      ...
      <div className="tool-group"><button ref={viewButton} aria-expanded={viewMenuOpen} aria-haspopup="menu" onClick={() => setViewMenuOpen(value => !value)}><Grid3X3 />{t('figureViewMenu')}</button><Popover anchorRef={viewButton} open={viewMenuOpen} onClose={() => setViewMenuOpen(false)} label={t('figureViewMenu')}><Menu label={t('figureViewMenu')} onClose={() => setViewMenuOpen(false)}><MenuItem onSelect={() => { setSnapToGrid(value => !value); setViewMenuOpen(false); }}><Grid3X3 />{snapToGrid ? t('hideGrid') : t('showGrid')}</MenuItem><MenuItem disabled={!state.nodes.length} onSelect={() => { alignAllNodes(); setViewMenuOpen(false); }}><LayoutGrid />{t('arrangeGrid')}</MenuItem><MenuSeparator /><MenuItem onSelect={() => { setTimelineOpen(value => !value); setViewMenuOpen(false); }}><Clock3 />{timelineOpen ? t('hideTimeline') : t('showTimeline')}</MenuItem><MenuItem onSelect={() => { setJourneyOverlayOpen(value => !value); setViewMenuOpen(false); }}><MapPin />{journeyOverlayOpen ? t('hidePaths') : t('showPaths')}</MenuItem></Menu></Popover></div>
      <div className="tool-group"><button disabled={!canUndo} onClick={onUndo} aria-label={t('undoDiagram')} title={`${t('undoDiagram')} · ${keys('Z')}`}><Undo2 /></button><button disabled={!canRedo} onClick={onRedo} aria-label={t('redoDiagram')} title={`${t('redoDiagram')} · ${keys('Z', { shift: true })}`}><Redo2 /></button></div>
    ...
    <div className="figure-layout">
      <div className={`flow-area zoom-${zoomTier} ${connecting ? 'is-connecting' : ''} ${playing ? 'timeline-playing' : ''}`}>
        {connecting && <div className="mode-banner" role="status"><Link2 /><span>{t('connectModeHint')}</span><button onClick={() => setConnecting(false)}><X /><span className="sr-only">{t('cancel')}</span></button></div>}
  ...
  return <>
    ...
    <div className="panel-body">
      {tab === 'card' && <>
        ...
        {activeMomentId && figure.type !== 'ort' && figure.type !== 'konzept' && <button className={`timeline-life-action ${figure.diedMomentId === activeMomentId ? 'active' : ''}`} onClick={() => onPatch({ diedMomentId: figure.diedMomentId === activeMomentId ? undefined : activeMomentId })}><Skull />{figure.diedMomentId === activeMomentId ? t('removeDeathMarker') : t('diesHere')}</button>}
    ...
    {deleteEdge && <ConfirmDialog title={t('deleteConnection')} description={t('deleteConnectionDescription', { name: deleteEdge.name })} confirmLabel={t('deleteConnection')} undoable onConfirm={() => onState({ ...state, edges: state.edges.filter(item => item.id !== deleteEdge.edge.id) })} onClose={() => setDeleteEdge(null)} />}
  </>;
}
