...
def _observe_failure(app, event: str, exc: Exception) -> None:
    """Record only bounded diagnostics; never return provider text or paths."""

    app.observability.logger.event("warning", event, error_type=type(exc).__name__)


@get("/api/backup", world=True)
def status(handler, request: Request, app) -> None:
    with app.lock:
        handler.send_json(app.backups.status(request.world.backup))
...
def remote_worlds(handler, request: Request, app) -> None:
    ...
    endpoint = app.backups.default_endpoint()
    if not endpoint:
        return handler.send_exception(BackupEndpointNotConfigured())
    try:
        authorization = app.backup_authorization(endpoint, request.session)
        handler.send_json(
            {
                "ok": True,
                "endpoint": endpoint,
                "worlds": app.backups.list_remote_worlds(endpoint, authorization),
            }
        )
    except ApplicationError as exc:
        _observe_failure(app, "backup.remote_worlds_failed", exc)
        handler.send_exception(exc)
    except Exception as exc:
        _observe_failure(app, "backup.remote_worlds_failed", exc)
        handler.send_exception(BackupGatewayError(params={"operation": "list_remote_worlds"}))
...
def snapshot(handler, request: Request, app) -> None:
    ...
    try:
        authorization = (
            app.backup_authorization(world.backup.endpoint_url, request.session) if push else None
        )
    except (PermissionError, RuntimeError):
        return handler.send_exception(
            BackupAuthorizationUnavailable(params={"operation": "upload"})
        )
    try:
        with app.lock:
            result = app.backups.commit(world.backup, message, push, authorization)
    except ApplicationError as exc:
        _observe_failure(app, "backup.snapshot_failed", exc)
...
def restore_from_endpoint(handler, request: Request, app) -> None:
    ...
    if not app.worlds.is_valid_id(world_id):
        return handler.send_exception(BackupRequestInvalid(params={"field": "worldId"}))
    endpoint = app.backups.default_endpoint()
    if not endpoint:
        return handler.send_exception(BackupEndpointNotConfigured())
    try:
        ...
        _observe_failure(app, "backup.remote_restore_failed", exc)
        handler.send_exception(exc)
    except Exception as exc:
        _observe_failure(app, "backup.remote_restore_failed", exc)
        handler.send_exception(BackupGatewayError(params={"operation": "restore_remote"}))
...
def restore_local(handler, request: Request, app) -> None:
    try:
        ...
        _observe_failure(app, "backup.local_restore_request_invalid", exc)
        return handler.send_exception(BackupRequestInvalid(params={"operation": "restore_local"}))
    world = handler.world_from_body(request.session, payload)
    if world is None:
        return
    try:
...
#     token does not have (src/quiltor/infrastructure/backup/login.py files tokens by endpoint).
...
#     that user (src/quiltor/modules/identity/service.py).
...
def _redirect_uri(handler) -> str:
    ...
    would be wrong the first time somebody runs `python3 apps/web/server.py 8080`.
...
def _endpoint_for(request: Request, app) -> str:
    ...
    if world_id:
        try:
            ...
            return app.backups.endpoint_for_world(world_id, request.session.sub)
        except Exception:
            pass
    return app.backups.default_endpoint()
        ...
        return handler.send_json(
            {
                ...
                "email": session.email,
                "name": session.name,
                "issuer": app.identity.auth.issuer,
                "scope": "",
                ...
                **({} if usable else {"reasonCode": "backup.session_token_unavailable"}),
            }
        )
    handler.send_json(
        {
            ...
            "configured": True,
            "hosted": False,
            **app.backups.login_status(endpoint),
        }
    )
...
def login_begin(handler, request: Request, app) -> None:
    ...
    """
    if app.identity.multi_user:
        # There is no browser flow here to start -- see login_status.  This is a
        # rejected operation, so it uses the same structured non-2xx boundary as
        # every other application failure.
        return handler.send_api_error(
            409,
            error_code="backup.login_not_applicable",
            retryable=False,
        )
    endpoint = _endpoint_for(request, app)
    if not endpoint:
        return handler.send_exception(BackupEndpointNotConfigured())
    try:
        authorize_url = app.backups.begin_login(endpoint, _redirect_uri(handler))
    except Exception as exc:
        ...
        _observe_failure(app, "backup.login_start_failed", exc)
        return handler.send_exception(BackupGatewayError(params={"operation": "login"}))
    handler.send_json(
        {
            "ok": True,
            "endpoint": endpoint,
...
def login_end(handler, request: Request, app) -> None:
    ...
    if endpoint and not app.identity.multi_user:
        app.backups.end_login(endpoint)
...
def login_callback(handler, request: Request, app) -> None:
    ...
    try:
        app.backups.complete_login(code, state, _redirect_uri(handler))
        ...
        _observe_failure(app, "backup.login_callback_rejected", exc)
        return _page(handler, "Die Anmeldung ist ungültig oder abgelaufen.", 400)
    except Exception as exc:
        _observe_failure(app, "backup.login_callback_failed", exc)
        return _page(handler, "Der Anbieter konnte die Anmeldung nicht abschließen.", 502)
