...
from quiltor.infrastructure.inference.runtimes import resolve_port, spawn_logged
...
def start(
    ...
    Mirrors src/quiltor/infrastructure/inference/runtimes/llamacpp.py's start() shape exactly, so
    src/quiltor/infrastructure/inference/select.py can treat every runtime interchangeably. Here,
    ...
    `python3 -m quiltor.infrastructure.inference.installer --runtime mlx` directly), which is a
    ...
    python = (
        ...
        if binary_override
        else base / "runtime" / "mlx-venv" / "bin" / "python3"
    )
    bridge = resources.sidecar_asset("inference/mlx/bridge.py")
    models_dir = base / "models" / "mlx"
    models = (
        sorted(p for p in models_dir.glob("*") if (p / "config.json").exists())
