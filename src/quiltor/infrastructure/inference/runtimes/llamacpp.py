...
from quiltor.infrastructure.inference.runtimes import (
    bundled_runtime_dir,
    resolve_port,
    spawn_logged,
)
...
def resolve_binary(base: Path) -> Path:
    ...
    (Mac App Store, see src/quiltor/infrastructure/platform/runtime_target.py), otherwise the <base>/runtime
    directory that src/quiltor/infrastructure/inference/installer.py downloads into.
...
def start(
    ...
    src/quiltor/infrastructure/inference/installer.py), not an error. Otherwise returns the spawned
