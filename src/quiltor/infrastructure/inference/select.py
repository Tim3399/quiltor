...
from quiltor.infrastructure.inference.runtimes import llamacpp, mlx
from quiltor.infrastructure.platform.runtime_target import is_store_distribution
from quiltor.infrastructure.platform.system import is_apple_silicon
...
def _preference_order() -> tuple[Runtime, ...]:
    ...
    # memory) but only starts if it was actually installed (quiltor.infrastructure.inference.installer
    ...
    # never installed.
    # A Store build has no MLX venv to start -- src/quiltor/infrastructure/inference/installer.py refuses to
    ...
    if not is_store_distribution() and is_apple_silicon():
        return (mlx, llamacpp)
    return (llamacpp,)
...
def start_runtime(base: Path, data: Path, url: str) -> tuple[subprocess.Popen[str], Path] | None:
    ...
    src/quiltor/infrastructure/inference/runtimes/llamacpp.py -- neither src/quiltor/modules/assistant.py nor
    src/quiltor/infrastructure/inference/shared/contract.py need to change, since both only ever
