...
from quiltor.infrastructure.pdf.tokens import RENDER_TOKEN_TTL
...
def python_package_renderer() -> PdfRenderer:
    """PDF capability of the portable Python artifact.

    The base install stays small and reports a typed unavailable result.  The
    explicit ``browser-pdf`` extra pins the verified Playwright runtime and
    enables an installed Chrome/Edge without requiring a system Node.js.
    """

    return (
        system_browser.render
        if importlib.util.find_spec("playwright") is not None
        else unavailable.render
    )
...
def desktop_renderer_name(os_name: str | None = None, sandboxed: bool | None = None) -> str:
    ...
    see distribution/desktop/pyinstaller/bundle.py, which drops Playwright wherever this is not
    SYSTEM_BROWSER.
