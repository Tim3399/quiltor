...
def _ordinal(year: int, month: int, day: int, months: list[Mapping[str, Any]]) -> int:
    ...
    return (
        (year - 1) * year_length + sum(item["dayCount"] for item in months[: month - 1]) + day - 1
    )


def time_to_calendar_date(time_value: int, system: Mapping[str, Any]) -> CalendarDate | None:
    ...
    if kind == "gregorian":
        ...
        return CalendarDate(
            ...
            projected.day,
            projected.weekday(),
            projected.strftime("%B"),
            projected.strftime("%b"),
            projected.strftime("%A"),
            projected.strftime("%a"),
        )
    if kind != "custom":
    ...
    return CalendarDate(
        ...
        day_of_year + 1,
        weekday,
        str(month_item.get("name", "")),
        str(month_item.get("shortName", "")),
        str(weekday_item.get("name", "")),
        str(weekday_item.get("shortName", "")),
    )
...
def format_time(time_value: int, system: Mapping[str, Any]) -> str:
    ...
    values = _FormatValues(
        ...
        day=projected.day,
        monthName=projected.month_name,
        monthShortName=projected.month_short_name,
        weekdayName=projected.weekday_name,
        weekdayShortName=projected.weekday_short_name,
        eraName=era_name,
        eraAbbreviation=era_abbreviation,
        era=era_abbreviation or era_name,
        time=time_value,
    )
