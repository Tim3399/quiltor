...
`quiltor` (or `quiltor run`) starts the server exactly like `python3 apps/web/server.py`
...
# minted fresh per process and lives only in memory (src/quiltor/modules/identity/service.py's
...
def _version() -> str:
    # Mirrors the web host's VERSION lookup without importing that
    # heavy, side-effectful composition root just for this.
...
def _start_server(port: int, no_open: bool, print_token: bool) -> None:
    _load_config()
    os.environ.setdefault("QUILTOR_RUNTIME_HOST", "cli")
    from quiltor.hosts.web import server
    from quiltor.infrastructure.pdf import python_package_renderer

    server.run(
        port=port,
        no_open=no_open,
        print_token=print_token,
        application=server.build_web_application(render_pdf=python_package_renderer()),
    )
...
def run(
    ...
    """Start the server (same as `python3 apps/web/server.py`)."""
...
def install() -> None:
    ...
    typer.echo()
    _install_writing_assistance_step()
    typer.echo()
    _install_llm_step()
...
def _install_llm_step() -> None:
    ...
    installation = build_assistant_installation(
        build_feature_availability(process_role=ProcessRole.CLI)
    )
    try:
        installation.install_selected("auto")
...
def _install_writing_assistance_step() -> None:
    ...
    service = build_writing_assistance_service(
        data_dir,
        build_feature_availability(process_role=ProcessRole.CLI),
    )
    try:
