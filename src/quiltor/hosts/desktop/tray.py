...
difference internally, so nothing here branches on the OS (the infrastructure
platform adapters are where the few things that still have to do it live).
...
from quiltor.infrastructure.platform.system import (
...
def start_tray_icon(
    ...
    if not TRAY_SUPPORTS_BACKGROUND_THREAD:
        ...
        # would break app startup outright; see src/quiltor/infrastructure/platform/adapters/macos.py for the
