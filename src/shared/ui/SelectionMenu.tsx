...
export function SelectionMenu({ anchorRef, open, label, actions, onClose }: { anchorRef: RefObject<HTMLElement | null>; open: boolean; label: string; actions: SelectionAction[]; onClose: () => void }) {
  return <Popover anchorRef={anchorRef} open={open} onClose={onClose} label={label}><Menu label={label} onClose={onClose}>{actions.map(action => <MenuItem key={action.id} disabled={action.disabled} onSelect={() => { action.run(); onClose(); }}>{action.label}</MenuItem>)}</Menu></Popover>;
}
