...
export function CommandPalette({ open, label, inputLabel = label, placeholder, emptyLabel, items, onClose }: {
  open: boolean; label: string; inputLabel?: string; placeholder: string; emptyLabel: string; items: CommandPaletteItem[]; onClose: () => void;
}) {
  ...
  const results = useMemo(() => {
    ...
    return items.filter(item => (!item.requiresQuery || needle) && (!needle || [item.label, item.detail, ...(item.keywords || [])].some(value => value?.toLocaleLowerCase().includes(needle))));
  }, [items, query]);
  if (!open) return null;
