...
export function SegmentedControl<T extends string>({ label, value, options, onChange }: { label: string; value: T; options: Segment<T>[]; onChange: (value: T) => void }) {
  ...
  const move = (event: KeyboardEvent, direction: number | 'first' | 'last') => {
    const buttons = [...(root.current?.querySelectorAll<HTMLButtonElement>('[role="radio"]:not(:disabled)') || [])];
  ...
  return <div ref={root} className="ui-segmented" role="radiogroup" aria-label={label} onKeyDown={event => {
  ...
  }}>{options.map(option => <button key={option.value} type="button" role="radio" aria-checked={value === option.value} tabIndex={value === option.value ? 0 : -1} disabled={option.disabled} onClick={() => onChange(option.value)}>{option.label}</button>)}</div>;
}
