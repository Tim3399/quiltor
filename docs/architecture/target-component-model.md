...
| Storyboard         | The fifth workspace now edits the separate v1 `StoryboardDocument` through its own history, autosave and gateway. ReactFlow renders boards, note/reference/board/group nodes and planning edges; normalized SQLite persistence owns an independent revision. The client derives exact-card backlinks without changing canon. | Keep that planning aggregate behind a context application port as operations become more focused. AI planning context and explicit overlap ordering remain later slices and must not couple the document to `StoryWorld` or `WorldState(t)`. |
