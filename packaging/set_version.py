...
def apply_version(version: str, repo_root: Path = REPO_ROOT) -> list[str]:
    ...
    parse_version(version)  # never write something that is not a version

    (repo_root / VERSION_FILE).write_text(version + "\n", encoding="utf-8")
    ...
    _rewrite_json(repo_root / PACKAGE_JSON, set_package_version)
    _rewrite_json(repo_root / PACKAGE_LOCK, set_lock_version)
    return [VERSION_FILE, PACKAGE_JSON, PACKAGE_LOCK]
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        target = next_version(current, args.version)
    ...
    if dirty:
        print("error: the working tree has uncommitted changes.", file=sys.stderr)
        print("       A version bump should be one reviewable commit of its own.", file=sys.stderr)
        print("       Commit or stash these first:", file=sys.stderr)
    ...
    written = apply_version(target, repo_root)
